...
public class ManagerEarningsReportController implements Initializable {
	   ...
	   @FXML // fx:id="managerLbl"
	    ...
	    {	
	    	 if((storeNameTxt1.getText().contentEquals("LilacHaifa") == true ||
	    		 ...
	    		 LilacApp.client.accept("get earn " + managerLbl.getText() + " " + storeNameTxt1.getText()
	  	             + " " + storeNameTxt2.getText() + " " + month1Txt.getText() + " " + month2Txt.getText());
	  	             String response = LilacApp.client.getResponse();
	  	             
	  	             
	  	             if(response.contentEquals("Unsuccessful Earnings Comparison") == true)
	  	             {
	  	             ...
	  	             else
	  	             {
	  	            	String[] temp = response.trim().split("-");
		  	             ...
		  	             result1Lbl.setText("Result1 : " + temp[0]);
		  	             result2Lbl.setText("Result2 : " + temp[1]);
	  	             }
	  	   	  }
	  	       else
	  	       {
	    ...
	    {   
	        if(storeNameTxt.getText().contentEquals("LilacHaifa") == true ||
	           ...
	           storeNameTxt.getText().contentEquals("All") == true)
	   	    {
	        	...
	        	LilacApp.client.accept("get histEarnings " + managerLbl.getText() + " " + storeNameTxt.getText());
	             String response = LilacApp.client.getResponse();
	             
	             
	             if(response.contentEquals("Unsuccessful Earnings Histogram") == true)
  	             {
  	            	checkTxt.setText("Error! Manager does not have the permission for this action"); 
  	             }
  	             else
  	             {
  	            	String[] temp = response.trim().split("-");
  		             ...
  		             Q1Lbl.setText(temp[0]);
  		             Q2Lbl.setText(temp[1]);
  		             Q3Lbl.setText(temp[2]);
  		             Q4Lbl.setText(temp[3]); 
  	             }
	   	  }
	       else
	       {
